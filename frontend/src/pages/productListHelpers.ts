...
export const filterCollectionProducts = (
  ...
  if (options.usingServerPagination || (!collection && !keyword)) return products;
  const normalizedKeyword = keyword.toLowerCase();
  return products.filter((product) => {
    if (collection === 'smart-devices' && !matchesSmartDeviceCollection(product)) return false;
    if (collection && normalizedKeyword && !productSearchText(product).includes(normalizedKeyword)) return false;
    return true;
  });
};
...
export const filterProductsByRefinements = (
  ...
  const petSizes = filters.petSizes.map((size) => size.toLowerCase());
  const materials = filters.materials.map((material) => material.toLowerCase());
  const colors = filters.colors.map((color) => color.toLowerCase());
  ...
  return products.filter((product) => {
    ...
    const matchSize = petSizes.length === 0 || petSizes.some((size) => specText.includes(size));
    const matchMaterial = materials.length === 0 || materials.some((material) => specText.includes(material));
    const matchColor = colors.length === 0
      || colors.some((color) => specText.includes(color) || productName.includes(color));
...
export const getPersonalizedSortScore = (
  ...
  ((context.recentProductIdSet?.has(product.id) ?? context.recentProductIds.includes(product.id)) ? 6 : 0) +
...
export const getConversionSortScore = (
  ...
  return personalizedScore
    ...
    + (savings > 0 ? Math.min(12, savings / 20) : 0)
...
export const sortProductList = (
  ...
  const decorated = products.map((product, index) => {
    const personalizedScore = getPersonalizedSortScore(product, context);
    return {
      product,
      index,
      ...
      personalizedScore,
      conversionScore: getConversionSortScore(product, context, personalizedScore),
    };
  });
  decorated.sort((a, b) => {
...
export const pickRecommendedProduct = (
  ...
  let recommended: Product | null = null;
  let recommendedScore = Number.NEGATIVE_INFINITY;
  products.forEach((product) => {
    if (isProductSoldOut(product)) return;
    const score = getPersonalizedSortScore(product, context);
    if (!recommended || score > recommendedScore) {
      recommended = product;
