...
const forgotPasswordPanelSource = readSource('forgotPasswordPanels.tsx');
...
describe('Storefront brand i18n contracts', () => {
  it('keeps visible storefront brand marks translated instead of hardcoded', () => {
    ...
    expect(forgotPasswordPanelSource).toContain('<div className="shopee-login-mark">{t(\'common.brand\')}</div>');

    expect(navbarSource).not.toContain('ShopMX');
    expect(forgotPasswordPanelSource).not.toContain('ShopMX');
  });
