...
describe('Home render memoization contracts', () => {
  ...
  it('keeps expensive and reusable Home derived collections memoized', () => {
    ...
    expect(homeSource).toMatch(/const personalizedDisplayMetrics = useMemo\(/);
    expect(homeSource).toContain('const personalizedReadyProducts = personalizedDisplayMetrics.readyProducts;');
    expect(homeSource).toContain('const personalizedDealCount = personalizedDisplayMetrics.dealCount;');
