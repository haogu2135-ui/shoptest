...
${recoveryAdds}`;
  ...
  it('keeps saved-for-later and guest-cart storage changes synchronized across tabs', () => {
    ...
    const session = readCartSessionDataSource();
    const storageStart = session.indexOf('const refreshStoredState = (event: Event) => {');
    ...
    expect(storageSource).toContain('const isStorageEvent = event.type === \'storage\';');
    expect(storageSource).toContain('const storageEvent = isStorageEvent ? event as StorageEvent : null;');
    expect(storageSource).toContain('const allStorageCleared = storageEvent?.key === null;');
    expect(storageSource).toContain('const savedItemsChanged = event.type === \'shop:save-for-later-updated\'');
    expect(storageSource).toContain('setSavedItems(getSavedForLaterItemsSnapshot());');
    expect(storageSource).toContain('const guestCartChanged = event.type === \'shop:cart-updated\'');
    expect(storageSource).toContain("if (!guestCartChanged || getLocalStorageItem('token')) return;");
    ...
    expect(storageSource).toContain("window.addEventListener('shop:save-for-later-updated', refreshStoredState);");
    expect(storageSource).toContain("window.addEventListener('shop:cart-updated', refreshStoredState);");
    expect(storageSource).toContain("window.addEventListener('storage', refreshStoredState);");
    expect(storageSource).toContain("window.removeEventListener('storage', refreshStoredState);");
  });
