...
describe('PetGalleryManagement delete confirmation guards', () => {
  ...
  it('constrains long delete confirmation copy inside narrow admin viewports', () => {
    ...
    expect(f3523Css).toMatch(/\.pet-gallery-management-page__deletePopconfirm\.shop-mobile-popup-layer\s+\.ant-popconfirm-buttons\s+\.ant-btn\s*\{[\s\S]*?min-height:\s*44px;[\s\S]*?max-width:\s*100%;[\s\S]*?margin-inline-start:\s*0;[\s\S]*?white-space:\s*normal;/);
    expect(f3523Css).not.toMatch(/\.pet-gallery-management-page__deletePopconfirm\.shop-mobile-popup-layer\s+\.ant-popconfirm-buttons\s+\.ant-btn\s*\{[\s\S]*?min-height:\s*(?:3[0-9]|4[0-3])px/);
