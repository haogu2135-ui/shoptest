...
${addressActions}`;
  ...
  it('keeps payment polling lifecycle-bound without delayed location redirects', () => {
    ...
    const pollingEffectStart = source.indexOf('const pollPaymentState = useCallback(async () => {');
    ...
    expect(refreshStateSource).toContain('if (!requestIsActive()) return;');
    expect(pollingEffectSource).toContain('enabled: process.env.NODE_ENV !== \'test\' && paymentModalVisible && Boolean(selectedPaymentOrderId)');
    expect(pollingEffectSource).toContain('intervalMs: 5000');
    expect(pollingEffectSource).toContain('run: pollPaymentState');
    expect(pollingEffectSource).toContain('const pollPaymentState = useCallback(async () => {');
    expect(pollingEffectSource).toContain('await refreshPaymentState(selectedPaymentOrderId, isActive);');
    expect(pollingEffectSource).not.toContain('window.setInterval');
  });
  ...
  it('waits for payment channels before consuming payment-return sync state', () => {
    ...
    expect(channelEffectSource).toContain('const res = await paymentApi.getChannels({ signal: abortController.signal });');
  ...
  it('keeps profile payment channel failures recoverable from the payment modal', () => {
    ...
    const helpers = readProfileHelpersSource();
    expect(helpers).toContain("const retryPaymentChannelsActionLabel = `${t('common.retry')}: ${paymentOrderLabel} ${t('pages.checkout.paymentMethod')}`;");
    expect(source).toContain('buildProfileActionLabels({');
    expect(source).toContain('retryPaymentChannelsActionLabel');
