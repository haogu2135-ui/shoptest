...
## 2026-06-21 UTC Android build/runtime spot check

- 【Real issue】`frontend/src/utils/mobileUpdate.test.ts` still asserts the old manifest shape for the public APK. The test expects `publicManifest.apkUrl` to be `/downloads/${fileName}?v=${versionName}` and `fileName` to be `shoptest.apk`, but `frontend/scripts/generate-mobile-version.js` now generates a versioned artifact URL (`/downloads/shoptest-1.0.173.apk`) and uses `legacyApkUrl` for the cache-busted stable alias. I verified the mismatch by running `npm test -- --runInBand frontend/src/utils/mobileUpdate.test.ts`; the suite fails on that assertion. This is a real validation issue because it blocks the Android release metadata contract from being checked correctly.
- 【Maintainer follow-up 2026-06-27】Current source is resolved by existing changes: `frontend/src/utils/mobileUpdate.test.ts` now expects `apkUrl` to equal `/downloads/${publicManifest.fileName}`, `legacyApkUrl` to equal `/downloads/shoptest.apk?v=${publicManifest.versionName}`, and `fileName` to equal `shoptest-${publicManifest.versionName}.apk`. This matches `frontend/public/downloads/mobile-version.json` and `frontend/scripts/generate-mobile-version.js`. Verification was source inspection only; no Jest/Appium/device run was performed.
- 【Non-issue】`mobile-tests/appium/config/wdio.conf.js` points Appium at `frontend/public/downloads/shoptest.apk`, which looks stale at first glance because the manifest now prefers the versioned APK path. In this checkout, however, `frontend/public/downloads/shoptest.apk` and `frontend/public/downloads/shoptest-1.0.173.apk` are byte-identical and have the same SHA-256, so the Appium harness still installs a valid Android package. The stable alias is currently intact, so this is not a delivery break.
